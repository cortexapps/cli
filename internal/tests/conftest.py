...
def pytest_collection_modifyitems(items):
    """Auto-apply the 'functional' marker and xdist group to every test.

    Tests are grouped by their fixture dependencies so that session-scoped
    fixtures (which create ephemeral repos/projects) are shared within a
    single xdist worker rather than duplicated per worker.
    """
    functional = pytest.mark.functional
    for item in items:
        # Don't auto-mark cleanup tests as functional
        if any(m.name == "cleanup" for m in item.iter_markers()):
            continue
        item.add_marker(functional)

        # Assign xdist groups based on fixture dependencies
        fixture_names = item.fixturenames
        if "gh_test_repo" in fixture_names or "gh_default_sha" in fixture_names:
            item.add_marker(pytest.mark.xdist_group("gh_repo"))
        elif "gl_test_project" in fixture_names or "gl_test_namespace" in fixture_names:
            item.add_marker(pytest.mark.xdist_group("gl"))
        elif "gh_test_org" in fixture_names:
            item.add_marker(pytest.mark.xdist_group("gh_org"))

from tests.gh_helpers import (
...
def import_functional_workflows():
    ...
    workflow_dir = os.path.join(os.path.dirname(__file__), "..", "..", "data", "functional", "workflows")

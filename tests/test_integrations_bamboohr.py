...
def _dummy_file(tmp_path):
    f = tmp_path / "test_integrations_bamboohr_add.json"
    f.write_text("foobar")
    return f

@responses.activate
def test_integrations_bamboohr_add(tmp_path):
    f = _dummy_file(tmp_path)
    responses.add(responses.POST, os.getenv("CORTEX_BASE_URL") + "/api/v1/bamboohr/configuration", json={}, status=200)
    cli(["integrations", "bamboohr", "add", "-f", str(f)])

@responses.activate
def test_integrations_bamboohr_get():
    responses.add(responses.GET, os.getenv("CORTEX_BASE_URL") + "/api/v1/bamboohr/default-configuration", json={}, status=200)
    cli(["integrations", "bamboohr", "get"])

@responses.activate
def test_integrations_bamboohr_update(tmp_path):
    f = _dummy_file(tmp_path)
    responses.add(responses.PUT, os.getenv("CORTEX_BASE_URL") + "/api/v1/bamboohr/configuration", json={}, status=200)
    cli(["integrations", "bamboohr", "update", "-f", str(f)])

@responses.activate
def test_integrations_bamboohr_validate():

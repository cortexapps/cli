...
def test():
    ...
    response = json_response(["custom-metrics", "get", "-t", "shipping-integrations", "-k", "vulnerabilities"])
    assert response['data'][0]['value'] == 3.0, "should have single value of 3.0"
    ...
    response = json_response(["custom-metrics", "get", "-t", "shipping-integrations", "-k", "vulnerabilities"])
    assert response['total'] == 3, "should have total of 3 metrics data points"

...
def test_integrations_incidentio_add(tmp_path):
    ...
    responses.add(responses.POST, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configuration", json=[{'accountId': 123, 'alias:': 'test', 'isDefault': json.dumps("true"), 'personalKey': 'xxxx', 'region': 'US'}], status=200)
    cli(["integrations", "incidentio", "add", "-f", str(f)])

@responses.activate
def test_integrations_incidentio_add_multiple(tmp_path):
    f = _dummy_file(tmp_path)
    responses.add(responses.POST, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configurations", json=[{'accountId': 123, 'alias:': 'test', 'isDefault': json.dumps("true"), 'personalKey': 'xxxx', 'region': 'US'}], status=200)
...
def test_integrations_incidentio_get():
    responses.add(responses.GET, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configuration/test", json=[{'accountId': 123, 'alias:': 'test', 'isDefault': json.dumps("true"), 'personalKey': 'xxxx', 'region': 'US'}], status=200)
    cli(["integrations", "incidentio", "get", "-a", "test"])

@responses.activate
def test_integrations_incidentio_get_all():
    responses.add(responses.GET, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configurations", json=[{'accountId': 123, 'alias:': 'test', 'isDefault': json.dumps("true"), 'personalKey': 'xxxx', 'region': 'US'}], status=200)
    cli(["integrations", "incidentio", "get-all"])

@responses.activate
def test_integrations_incidentio_get_default():
    responses.add(responses.GET, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/default-configuration", json=[{'accountId': 123, 'alias:': 'test', 'isDefault': json.dumps("true"), 'personalKey': 'xxxx', 'region': 'US'}], status=200)
    cli(["integrations", "incidentio", "get-default"])

@responses.activate
def test_integrations_incidentio_update(tmp_path):
    f = _dummy_file(tmp_path)
    responses.add(responses.PUT, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configuration/test", json=[{'alias:': 'test', 'isDefault': json.dumps("true")}], status=200)
    cli(["integrations", "incidentio", "update", "-a", "test", "-f", str(f)])

@responses.activate
def test_integrations_incidentio_validate():
    responses.add(responses.POST, os.getenv("CORTEX_BASE_URL") + "/api/v1/incidentio/configuration/validate/test", json={'alias': 'test', 'isValid': json.dumps("true"), 'message': 'someMessage'}, status=200)
